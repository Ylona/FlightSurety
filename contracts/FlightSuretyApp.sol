...
contract FlightSuretyApp {
    ...
    */
    ...
    /**
* @dev Modifier that requires an airline to be the function caller
    */
    ...
    */
    ...
    returns (bool success, uint256 votes)
    {
        ...
        if (numberOfRegisteredAirlines < 4) {
            ...
            address[] memory consendingAirlines = flightSuretyData.getConsendingAirlines(newAirline);
            for (uint c = 0; c < consendingAirlines.length; c++) {
                if (consendingAirlines[c] == msg.sender) {
                    isDuplicate = true;
                    break;
                }
            }
            require(!isDuplicate, "Caller has already called this function.");
            if (consendingAirlines.length + 1 >= numberOfRegisteredAirlines / 2) {
                hasConsensus = true;
                flightSuretyData.resetConsendingAirlines(newAirline);
            } else {
                flightSuretyData.addConsendingAirlines(newAirline, msg.sender);
            }
        }
...
contract FlightSuretyData {
    ...
    function getConsendingAirlines(address wallet) external view returns (address[] memory);
